    ...
    <AppBar position="static" sx={{height:'64px', maxHeight:'64px', minHeight:'64px', width:'100%', position:'sticky', top:0, zIndex:998, displayPrint:'none' }}  >
      <Toolbar sx={{
        ...
        <Box display={'flex'} flexDirection={'row'} alignItems={'center'} position={'absolute'} bottom={0} left={0} ml={12}>
          <FiberManualRecord sx={{color: marketStatus.isOpen ? 'green' : 'red', width:0.17, mr:1}}  />
          <Typography variant='subtitle2' fontSize={{xs:12, sm:14}} sx={{ fontSize: '12px' }}>
              {t('menu.statusStr')}{marketStatus.isOpen ? t('menu.online') : t('menu.offline')}
          </Typography>
          </Box>

        <Box  justifyContent={'center'} mr={3} ml={'auto'}>
          <Typography variant='subtitle2' sx={{fontSize:{xs:12, sm:14}}}> {t('menu.storeNo')}: {marketStatus.marketNo} </Typography>
          <Typography variant='subtitle2' sx={{fontSize:{xs:12, sm:14}}}>{t('menu.counterNo')}: {marketStatus.counterNo} ({marketStatus.counterName})</Typography>
        </Box>
        <Box justifyContent={'center'}>
          <Typography variant='subtitle2' sx={{fontSize:{xs:12, sm:14}}}>{t('menu.counterIp')}: {marketStatus.counterIp}</Typography>
          <Typography variant='subtitle2' sx={{fontSize:{xs:12, sm:14}}}>{t('menu.version')}: {marketStatus.version}</Typography>
        </Box>
